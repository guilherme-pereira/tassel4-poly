 ...
 *
 * @version $Id: TreePainterQuant.java,v 1.2 2007/01/21 02:52:14 buckler4 Exp $
 *
 ...
 *     </ul>
 */
public class TreePainterQuant extends TreePainter {

    private Phenotype theCA;
    private int trait;
    private double traitMax,  traitMin;
    ...
    public TreePainterQuant(Tree toDisplay, String title, boolean showTitle) {
    ...
    public void setTrait(Phenotype ca, int trait) {
        ...
        traitMin = 9999999;
        double ct;
        for (int i = 0; i < theCA.getNumberOfTaxa(); i++) {
            ct = theCA.getData(i, trait);
    ...
    private Color getTraitColor(Node theNode) {
        if (theCA == null) {
            return null;
        }
        int taxa = theCA.getTaxa().whichIdNumber(theNode.getIdentifier().getName());
        if (taxa < 0) {
            return null;
        }
        double ct = theCA.getData(taxa, trait);
        if (Double.isNaN(ct)) {
            return null;
        }
        if (ct == Phenotype.MISSING) {
        ...
        double val = (ct - traitMin) / (traitMax - traitMin);
//          Color theColor=Color.getHSBColor((float)val, 1.0f, 1.0f);
        Color theColor = Color.getHSBColor(0.0f, 0.0f, (float) val);
//          System.out.println(theNode.getIdentifier().getName()+" val="+val);
        return theColor;
    }
        ...
        if (node.hasChildren()) {
            ...
            if (isUsingSymbols() && getTimeOrderCharacterData() != null) {
                ...
                if (isUsingColor()) {
                    ...
                    getNodeDisplay(node, defaultDisplay).display(g, name, p.x + (4 * XSPACER), p.y + (FONT_SIZE / 2));
                    Color c = getTraitColor(node);
                    if (c != null) {
                        g.setColor(c);
                        drawSymbol(g, p.x + XSPACER, p.y - (FONT_SIZE / 2), FONT_SIZE, 2);
                    }
                    g.setColor(NORMAL_LABEL_COLOR);
