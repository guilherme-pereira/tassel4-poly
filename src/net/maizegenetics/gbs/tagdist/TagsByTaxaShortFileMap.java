...
/**
 * A short implementation of TagByTaxa, maximum depth of tags is 32,767.
 ...
 * sequentially.
 * @author edbuckler
 * @author gabrielmargarido
 */
public class TagsByTaxaShortFileMap extends AbstractTagsByTaxa {
...
//    private int numLongPerTaxaDist=-1;
    ...
    public TagsByTaxaShortFileMap(String infile) {
        readDistFile(new File(infile), FilePacking.Short);
        bufferedTagDist = new byte[2 * taxaNum]; // Multiply taxaNum by 2 (short = 2 bytes per taxon)
        getReadCountForTagTaxon(0, 0);
    }

    /**
     * Creates an empty Short File Map
     * @param outFile
     * @param taxaNames
     * @param theTags 
     */
    public TagsByTaxaShortFileMap(String outFile, String[] taxaNames, Tags theTags) {
        int hapsOutput = 0;
        int tN = taxaNames.length;
        tagLengthInLong = theTags.getTagSizeInLong();
        byte[] obs = new byte[2 * tN]; // Multiply tN by 2 (short = 2 bytes per taxon)
        ...
        readDistFile(new File(outFile), FilePacking.Short);
        bufferedTagDist = new byte[2 * taxaNum]; // Multiply taxaNum by 2 (short = 2 bytes per taxon)
        getReadCountForTagTaxon(0, 0);
    }
    ...
    public void readDistFile(File inFile, FilePacking binary) {
        ...
        try {
            ...
            byteLenRow = 1 + (2 * taxaNum) + (8 * tagLengthInLong); // Multiply taxaNum by 2 (short = 2 bytes per taxon)
    ...
    public void setFastReadCountForAdjacentTagTaxonSet(int tagIndex, int firstTaxaIndex, byte[] values) {
        try {
            long pos = (byteLenRow * (long) tagIndex) + distStartPos + 2 * firstTaxaIndex; // 2 * firstTaxaIndex assumes short[] has been cast to byte[]
            theRAF.seek(pos);
            theRAF.write(values);
        } catch (IOException e) {
    ...
    private void setReadCountForTagTaxonRow(int tagIndex, int taxaIndex, int value) {
        ...
        if (value > Short.MAX_VALUE) {
            putValue = Short.MAX_VALUE;
        } else if (value < 0) {
            putValue = (short) 0;
        } else {
            putValue = (short) value;
        }
        ByteBuffer bb = ByteBuffer.wrap(bufferedTagDist);
        bb.putShort(2 * taxaIndex, putValue); // Multiply taxaIndex by 2 (short = 2 bytes per taxon)
        bufferChanged = true;
    }

    private void setReadCountForTagTaxonPoint(int tagIndex, int taxaIndex, int value) {
        try {
            long pos = (byteLenRow * (long) tagIndex) + distStartPos + 2 * taxaIndex; // Multiply taxaIndex by 2 (short = 2 bytes per taxon)
            theRAF.seek(pos);
            theRAF.write(value);
        } catch (IOException e) {
            System.out.println("Catch in reading bufferTagDist: " + e);
    ...
    public int getReadCountForTagTaxon(int tagIndex, int taxaIndex) {
        ...
        ByteBuffer bb = ByteBuffer.wrap(bufferedTagDist);
        return bb.getShort(2 * taxaIndex); // Multiply taxaIndex by 2 (short = 2 bytes per taxon)
    }
