 ...
 */
public class DiscoverySNPCallerV3Plugin extends AbstractPlugin {
    ...
    private static Logger myLogger = Logger.getLogger(DiscoverySNPCallerV3Plugin.class);
    TagsOnPhysicalMapV3 theTOPM = null;
    ...
    public DiscoverySNPCallerV3Plugin() {
        super(null, false);
    }

    public DiscoverySNPCallerV3Plugin(Frame parentFrame) {
...
//            myArgsEngine.add("-LocusBorder", "--locus-border", true);
        ...
        if (myArgsEngine.getBoolean("-m")) {
            ...
            inTOPMFileTest = null;
            theTOPM = new TagsOnPhysicalMapV3(inTOPMFile);
        } else {
            printUsage();
        ...
        if (myArgsEngine.getBoolean("-o")) {
            ...
            if (suppliedOutputFileName.contains(File.separator)) {
                ...
                if (!suppliedOutputFileName.substring(suppliedOutputFileName.lastIndexOf("/")).contains("+")) {
                    noWildCard = true;
                }
            }
            else if (!suppliedOutputFileName.contains("+")) {
            ...
            if (suppliedOutputFileName.contains(File.separator)) {
                outFolder = suppliedOutputFileName.substring(0,suppliedOutputFileName.lastIndexOf(File.separator));
            }
            else if (suppliedOutputFileName.contains("/")) {
                outFolder = suppliedOutputFileName.substring(0,suppliedOutputFileName.lastIndexOf("/"));
            }
...
class CustomSNPV3Log {
    ...
    public CustomSNPV3Log(String outHapMapFile, boolean append) {
        String logFileName;
...
class CustomSNPV3LogRecord {
    ...
    public CustomSNPV3LogRecord(int site, TagsAtLocus myTAL, int position, boolean[] isInbred, boolean includeReference) {
        ...
        majAllele= tagLocusStrand==-1? DiscoverySNPCallerV3Plugin.complementAllele(byteAlleles[0][site]) : byteAlleles[0][site];
        minAllele= tagLocusStrand==-1? DiscoverySNPCallerV3Plugin.complementAllele(byteAlleles[1][site]) : byteAlleles[1][site];
...
class TagLocusSiteQualityScoresV3 {
    ...
    public TagLocusSiteQualityScoresV3(int nSites) {
